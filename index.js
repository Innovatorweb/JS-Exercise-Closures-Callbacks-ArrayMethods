...
function getFullNames(runners) {
  ...
  runners.forEach( fullName => {
    lastFirstN.push (`${fullName.last_name}, ${fullName.first_name}`);
  });
  return lastFirstN;
}
...
function firstNamesAllCaps(runners) {
  ...
  runners.map ((runnerName, i) => {
    firstNCaps[i] = runnerName.first_name.toUpperCase();
  })
  return firstNCaps;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  let specTshirt = runners.filter ((runnerName) => {
...
function tallyUpDonations(runners) {
  let donationSum = runners.reduce ((total, runnerName) => {
    return total += runnerName.donation;
  }, 0);
  return donationSum;
}
